"""Ising model Hamiltonian implementation for Qiskit."""
...
class IsingHamiltonian:
    ...
    def build_hamiltonian(self) -> SparsePauliOp:
        """
        Build the Ising Hamiltonian as a Qiskit SparsePauliOp.
        ...
        for i in range(num_interactions):
            ...
            # Build Pauli string: I...I Z Z I...I
            # Qiskit uses little-endian ordering (rightmost qubit is index 0)
            pauli_str = ['I'] * self.num_qubits
            pauli_str[i] = 'Z'
            pauli_str[j] = 'Z'
            pauli_str = ''.join(reversed(pauli_str))  # Reverse for Qiskit convention

            pauli_list.append((pauli_str, -self.J))

        # Transverse field terms: -h * Σᵢ Xᵢ
        for i in range(self.num_qubits):
            # Build Pauli string: I...I X I...I
            pauli_str = ['I'] * self.num_qubits
            pauli_str[i] = 'X'
            pauli_str = ''.join(reversed(pauli_str))  # Reverse for Qiskit convention

            pauli_list.append((pauli_str, -self.h))

        return SparsePauliOp.from_list(pauli_list)

    def get_hamiltonian(self) -> SparsePauliOp:
        """Get the Hamiltonian SparsePauliOp."""
